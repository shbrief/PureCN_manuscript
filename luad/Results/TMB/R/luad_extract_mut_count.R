...
mut_list = lapply(seq_along(mut), function(i){
  ...
  data.frame(sampleID = mut[i], 
            TMB_tumor = mut_tumor$somatic.ontarget, 
            TMB_paired = mut_paired$somatic.ontarget)
}) 
...
write.table(mut_rate, file = file.path(out_dir, paste0(catalog, "_mut_count.tsv")))