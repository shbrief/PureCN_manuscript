...
loh_list = readRDS(file.path(out_dir, "LOH/data/ovc_LOH_all.rds"))

fname_convert = readRDS(file.path(out_dir, "LOH/data/ovc_name_convert.rds"))
fname_convert$file_name = gsub("_genes.csv", "", fname_convert$file_name)

for (i in seq_along(loh_list)) {
    ori_name = names(loh_list)[i]
    new_name = fname_convert$sample_name[which(fname_convert$file_name == ori_name)]
    names(loh_list)[i] = new_name
}
```
...
# OVC sampleID for downstream analysis 
ovc_sampleId = puri_ploi[which(puri_ploi$Sample == "OV"),]$SampleId
loh_list_pcn = loh_list[names(loh_list) %in% ovc_sampleId]   
```
...
ovc_abs_gl = abs_gl[names(abs_gl) %in% ovc_sampleId]   # 250 <-- 442 samples
...
a = CreateLOHmetadata(unlist(ovc_abs_gl)) 
...
```{r echo=FALSE}
# saveRDS(loh_list_pcn, "~/data2/PureCN_manuscript/Revision/LOH/data/LOH_list_OVC_purecn.rds")
# saveRDS(loh_list_abs, "~/data2/PureCN_manuscript/Revision/LOH/data/LOH_list_OVC_abs.rds")

loh_list_pcn = readRDS("~/data2/PureCN_manuscript/Revision/LOH/data/LOH_list_OVC_purecn.rds")
loh_list_abs = readRDS("~/data2/PureCN_manuscript/Revision/LOH/data/LOH_list_OVC_abs.rds")
```

```{r}
loh_summary = list()
for (i in ovc_sampleId) {
...
# saveRDS(loh_summary, "~/data2/PureCN_manuscript/Revision/LOH/data/OVC_LOH_summary.rds")

loh_summary = readRDS("~/data2/PureCN_manuscript/Revision/LOH/data/OVC_LOH_summary.rds")
```

Combine all the LOH information from 202 samples into one table.
```{r eval=TRUE}
test = data.frame()
...
Create a new table with the total sum of LOH pairs from ABSOLUTE and PureCN calls
```{r eval=TRUE}
...
ovc_loh_all = ggplot(t2, aes(x = loh.abs, y = loh.pcn)) +   # 33,195 data points
    ...
    ggtitle("E. TCGA-OV: Integer Copy Number") +
...
saveRDS(ovc_loh_all, file = file.path(out_dir, "Fig1E.ovc_loh_all.rds"))
```
