...
kit_subset = manifest_n[which(manifest_n$target_capture_kit_name == kit),]
...
res <- bplapply(allcalls_pon, system, BPPARAM = MulticoreParam(16))
...
allcalls_no_none <- lapply(seq_along(bed_subset$id), function(i){
  pre_fname <- tools::file_path_sans_ext(bed_subset[i,"filename"])
  mycall_no_none <- paste("nice -n 16 java -Xmx24g -jar",
                          gatk.jar,
...
res <- bplapply(allcalls_no_none, system, BPPARAM = MulticoreParam(16))

## Merge normal VCFs into one
merged.file = file.path(MUTECT_OUT, paste0(catalog, ".normals.merged.min5.vcf"))
merge.cmd = paste("java -Xmx24g -jar", gatk.jar, "-T CombineVariants -nt 4 --minimumN 5 --genotypemergeoption UNSORTED -R", REFERENCE)
merge.cmd = paste(merge.cmd, paste("--variant", dir(out.dir, pattern="^.*_pon_no_none\\.vcf$", full.names = TRUE), collapse=" "))
