...
## Load OVC benchmark output
```{r}
source("~/Documents/github/PureCN_manuscript/Figures/Final_Figures/non_dup.R")

benchmark = readRDS("~/Documents/github/PureCN_manuscript/Results/benchmark/data/all_ovc_benchmark.rds")
for (i in 2:10) {
    benchmark[, i] = as.numeric(benchmark[, i])
}
benchmark = benchmark[str_extract(benchmark$SampleId,"TCGA.{11}") %in% ovc_236,]   # 233 obs.
```
...
div_point = c(0.3, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9)
```
...
write.csv(summary, file = "~/Documents/github/PureCN_manuscript/Figures/Final_Tables/Table3.1_OV_AUC.csv")
```